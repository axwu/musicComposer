...
#ifndef STRUTILS_RETURN_TYPE
	typedef ATL::CString StrUtils_Return_Type;
#else
	typedef STRUTILS_RETURN_TYPE StrUtils_Return_Type;
#endif

#define PRINTNUM(BUF,SPEC,NUM)	_stprintf(BUF,SPEC,NUM)
...
template<>
inline StrUtils_Return_Type	ToString(const unsigned __int64& nNumber)
{
	return UnSignedNumber2Str(nNumber);
}
...
template<>
inline OIL::StrUtils_Return_Type OIL::ToString(const double& fNumber)
{
	return OIL::ToString((const float) fNumber);
}
...
{
	return _T("\"") + StrUtils_Return_Type(sz) + _T("\"");
}
template<>
...
{
	return _T("\"") + StrUtils_Return_Type(wsz) + _T("\"");
}
template<>
